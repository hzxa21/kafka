...
class LeaderElectionTest extends ZooKeeperTestHarness {
  ...
  def testLeaderElectionWithStaleControllerEpoch() {
    ...
    try {
      ...
      val requestBuilder = new LeaderAndIsrRequest.Builder(
        ApiKeys.LEADER_AND_ISR.latestVersion, controllerId, staleControllerEpoch, servers(1).brokerEpoch.get ,partitionStates.asJava, nodes.toSet.asJava)
  ...
  @Test
  def testLeaderElectionWithStaleBrokerEpoch() {
    // start 2 brokers
    val topic = "new-topic"
    val partitionId = 0
  ...
  private def staleBrokerEpochCallback(response: AbstractResponse): Unit = {
    val leaderAndIsrResponse = response.asInstanceOf[LeaderAndIsrResponse]
    staleBrokerEpochDetected = leaderAndIsrResponse.error match {
      case Errors.STALE_BROKER_EPOCH => true
      case _ => false
    }
  }
}
